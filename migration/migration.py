...
Reads the target PostgreSQL connection string from DATABASE_URL — checks
backend/.env and the repo-root .env, same as the running API (see
backend/backend/config.py) — or from --database-url.
"""
...
def resolve_database_url(explicit: str | None) -> str:
    ...
    if not url:
        sys.exit(
            "No DATABASE_URL set. Put your PostgreSQL connection string in "
